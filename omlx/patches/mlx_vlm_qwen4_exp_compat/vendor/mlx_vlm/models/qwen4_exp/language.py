...
class Qwen4ExpAttention(Qwen3_5Attention):
    ...
    def _gathered_text_verify_eligible(
        ...
        """Lightning MTP verify rows: batch-one contiguous text, aligned indexer state.

        The official verify path runs one dense SDPA per row over the whole
        prefix; above the QSA budget the gathered prefill arm attends only the
        selected blocks for all rows in one call. Rollback is unaffected: both
        arms append the same K/V and indexer rows, and ``QSAKVCache.trim``
        drops both.
        """
